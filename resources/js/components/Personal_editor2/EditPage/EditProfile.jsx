                ...
                <div id="profile-edit-content" className="info-box">
                    ...
                    <form id="basic-info-form">
                        ...
                        <div className="submit-row">
                            <button type="submit" className="submit-btn" onClick={() => { handleEditSubmit('儲存資料') }}>儲存資料</button>
                        </div>
                    </form>
                </div>
            ) : (
                <div id="profile-edit-content" className="info-box">
                    <button className="edit-button" onClick={() => { handleEditSubmit('編輯基本資料') }}>編輯</button>
                    <h3>基本資料</h3>
                    <hr />
                    {/* <!-- 姓名 --> */}
                    <div className="form-group">
            ...
            {edit === '編輯條件' ? (
                <div id="ideal-conditions" className="info-box">
                    ...
                    <form id="ideal-conditions-form">
                        ...
                        {/* <!-- 提交按鈕 --> */}
                        <div className="submit-row">
                            <button type="submit" className="submit-btn" onClick={() => { handleEditSubmit('儲存條件') }}>儲存條件</button>
                        </div>
                    </form>
                </div>
                ...
                <div id="ideal-conditions" className="info-box">
                    <button className="edit-button" onClick={() => { handleEditSubmit('編輯條件') }}>編輯</button>
                    <h3>理想接案條件</h3>
                    <hr />
                    {/* <!-- 理想接案地區 --> */}
                    <div className="form-group">
                        <label htmlFor="ideal-region">*理想接案地區：{userData.preferred_location}</label>
                    </div>
                    <hr />
                    {/* <!-- 理想接案類別 --> */}
                    <div className="form-group">
                        <label htmlFor="ideal-category">*接案類別：{userData.job_category}</label>
                        <span className="hint">【理想接案類別將顯示於個人工作室，可作為案主找尋人才時的重要依據】</span>
                    </div>
                    <hr />
                </div>
            ...
            {edit === '編輯經驗' ? (
                <div id="case-experience" className="info-box">
                    ...
                    <form id="case-experience-form">
                        ...
                        {/* <!-- 提交按鈕 --> */}
                        <div className="submit-row">
                            <button type="submit" className="submit-btn" onClick={() => { handleEditSubmit('儲存經歷') }}>儲存經歷</button>
                        </div>
                    </form>
                </div>
            ) : (
                <div id="case-experience" className="info-box">
                    <button className="edit-button" onClick={() => { handleEditSubmit('編輯經驗') }}>編輯</button>
                    <h3>接案經歷</h3>
                    <hr />
                    {/* <!-- 累積接案經驗 --> */}
                    <div className="form-group">
                        <label htmlFor="total-experience">累積接案經驗：{userData.accumulated_experience}</label>
                    </div>
                    <hr />
                    {/* <!-- 接案經驗條目 --> */}
                    <div className="form-group">
                        <label>接案經驗1：{userData.job_experience}{userData.job_description}</label>
                    </div>
                    <hr />
                </div>
            )}
