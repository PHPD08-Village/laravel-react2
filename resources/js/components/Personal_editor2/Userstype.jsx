...
const Userstype = () => {
    ...
    const handleRoleChange = (userRole) => { // 儲存點擊 接案者 or 業主
    ...
    return (
        <div className="sidebar">
            ...
            <div className="switcher">
                {/* 業主 */}
                <button className={`switch-btn ${userRole === '業主' ? 'active' : ''}`} onClick={() => handleRoleChange('業主')}>業主</button>                
                {/* 接案者 */}
                <button className={`switch-btn ${userRole === '接案者' ? 'active' : ''}`} onClick={() => handleRoleChange('接案者')}>接案者</button>
            </div>
            {/* 透過上面的按鈕來判斷顯示的內容 */}
            {userRole === '接案者' ? (
                // 點擊 接案者
